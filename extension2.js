...
class ExplorerProvider {
  ...
  async getParent(element) {
    ...
    // Stop at the panel's active boundary or active root
  ...
  async getChildren(element) {
    const folderUri = element ? element.resourceUri : (this.currentRootUri || this.boundaryRootUri || this.rootUri);
      if (!folderUri) return [];
      ...
      try {
        ...
        let items = files
          ...
          .map(([name, fileType]) => {
            ...
            // Retrieve or create cached node instance
...
function activate(context) {
  ...
  function chooseProviderForFile(fileUri) {
    ...
    // Sort by highest specificity (longest matching boundary root)
    providerCandidates.sort((a, b) => b.boundaryLength - a.boundaryLength);
    
    // Return ONLY the exact panel that owns the file scope
  ...
  async function revealFileInProvider(provider, tree, fileUri) {
    ...
    // Auto-reset view to boundary root if file is outside current subfolder navigation
    ...
    // Step through folder ancestors and reveal each node
  ...
  async function jumpToActiveEditor(editor = vscode.window.activeTextEditor) {
    ...
    if (panels.length === 0) return; // File is outside defined boundaries of both panels
  ...
  context.subscriptions.push(
    ...
    // Navigate Panel I view to selected folder
    ...
    // Set Panel I strict boundary to selected folder
    ...
    // Navigate Panel II view to selected folder
    ...
    // Set Panel II strict boundary to selected folder
